...
def get_commits_since_last_tag(tag):
    ...
    try:
        ...
        previous_tag = (
            tags[current_tag_index + 1] if current_tag_index + 1 < len(tags) else None
        )
...
def generate_release_notes(tag, repo, token, commits, previous_tag):
        ...
        if category_commits:
            ...
            for commit in category_commits:
                ...
                cleaned_message = re.sub(
                    r"^(feat|fix|docs|style|refactor|perf|test|chore|ci|build):\s*",
                    "",
                    message,
                    flags=re.IGNORECASE,
                )
                ...
                if pr_number and token:
                    ...
                    if pr_data:
                        pr_author = pr_data.get("user", {}).get(
                            "login", commit["author"]
                        )
                        ...
                        notes.append(
                            f"- {cleaned_message} ({sha}) by {commit['author']}"
                        )
                else:
    ...
    notes.append("")
    notes.append(
        "For more details, see the [full changelog](https://github.com/{}/compare/{}...{})".format(
            repo, previous_tag if previous_tag else "main", tag
        )
    )
...
def main():
    ...
    release_notes = generate_release_notes(
        args.tag, args.repo, args.token, commits, previous_tag
    )
